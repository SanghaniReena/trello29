...
app.post("/:idcards/cardsdetail", (req, res) => {
    ...
    mysqlConnection.query("UPDATE cards SET cDesc=?,cComment=?,cWatch=? WHERE idcards=?",[req.body.cDesc,req.body.cComment,req.body.cWatch,req.params.idcards] ,(err, rows) => {
        if (!err) {
            mysqlConnection.query("SELECT * FROM cards WHERE idcards=?", [req.params.idcards], (err, result) => {
                if (!err) {
                    res.send(result)
